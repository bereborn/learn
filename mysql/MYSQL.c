...
事务隔离
Read uncommitted
脏读问题: 一个事务可以读取另一个未提交事务的数据(程序员看到老板还没提交事务时的3.9w数据)

Read committed
不可重复读: 事务开启, 允许其他事务的UPDATE修改操作(程序员看到3.6w妻子要把钱全部转出充当家用并提交)
...
MVCC(多版本并发控制机制)
作用: 事务型存储引擎使用MVCC可以在大多数情况下代替行级锁能降低其系统开销, 实现了非阻塞的读操作, 写操作也只锁定了必要的行
...
redo log(重做日志): InnoDB特有的日志, redo log保证数据库异常重启之前提交的记录不会丢失(crash-safe), WAL技术(先写日志再写磁盘)
binlog(归档日志): server层日志, 记录了MySQL对数据库执行更改的所有操作, 没有crash-safe能力, 采用追加写的模式
bin log 三种模式
...
数据恢复到某个时间点的状态: 找到最近的一次全量备份, 从这个备份恢复到某个时间点的状态
